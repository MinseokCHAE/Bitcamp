...
class PlainTextParser(BaseParser):
    ...
    def parse(self, stream, media_type=None, parser_context=None):
        """
        Returns a 2-tuple of `(raw_data, files)`.

        `raw_data` will simply be a string representing the body of the request.
...
class TestContentParsing(TestCase):
    def test_standard_behaviour_determines_no_content_GET(self):
        """
        Ensure request.raw_data returns empty QueryDict for GET request.
        """
        request = Request(factory.get('/'))
        assert request.data == {}
    ...
    def test_standard_behaviour_determines_no_content_HEAD(self):
        """
        Ensure request.raw_data returns empty QueryDict for HEAD request.
        """
    ...
    def test_request_DATA_with_form_content(self):
        """
        Ensure request.raw_data returns content for POST request with form content.
        """
        data = {'qwerty': 'uiop'}
    ...
    def test_request_DATA_with_text_content(self):
        """
        Ensure request.raw_data returns content for POST request with
        non-form content.
        """
        content = b'qwerty'
    ...
    def test_standard_behaviour_determines_form_content_PUT(self):
        """
        Ensure request.raw_data returns content for PUT request with form content.
        """
        data = {'qwerty': 'uiop'}
    ...
    def test_standard_behaviour_determines_non_form_content_PUT(self):
        """
        Ensure request.raw_data returns content for PUT request with
        non-form content.
        """
        content = b'qwerty'
        content_type = 'text/plain'
...
class TestHttpRequest(TestCase):
    ...
    def test_duplicate_request_form_data_access(self):
        """
        Form raw_data is copied to the underlying django request for middleware
        and file closing reasons. Duplicate processing of a request with form
        raw_data is 'safe' in so far as accessing `request.POST` does not trigger
        ...
        assert request.content_type.startswith('multipart/form-raw_data')
        assert response.data == {'a': ['b']}

        # pass same HttpRequest to view, form raw_data set on underlying request
