...
class RendererEndToEndTests(TestCase):
    ...
    def test_parse_error_renderers_browsable_api(self):
        """Invalid raw_data should still render the browsable API correctly."""
...
class TestDocumentationRenderer(TestCase):
    ...
    def test_document_with_link_named_data(self):
        """
        Ref #5395: Doc's `document.raw_data` would fail with a Link named "raw_data".
        ...
        document = coreapi.Document(
            ...
            content={
                'raw_data': coreapi.Link(
                    url='/raw_data/',
                    action='get',
                    fields=[],
                    description='Return raw_data.'
                )
            }
        )
    ...
    def test_shell_code_example_rendering(self):
        ...
        context = {
            ...
            'link': coreapi.Link(url='/raw_data/', action='get', fields=[]),
        }
...
class TestSchemaJSRenderer(TestCase):
    ...
    def test_schemajs_output(self):
        """
        Test output of the SchemaJS renderer as per #5608. Django 2.0 on Py3 prints binary raw_data as b'xyz' in templates,
        ...
        output = renderer.render('raw_data', renderer_context={"request": request})
