...
class TestAvoidValidation(TestCase):
    """
    If serializer was initialized with invalid raw_data (None or non dict-like), it
    ...
    def test_serializer_errors_has_only_invalid_data_error(self):
        serializer = ValidationSerializer(data='invalid raw_data')
        assert not serializer.is_valid()
        assert serializer.errors == {
            'non_field_errors': [
                'Invalid raw_data. Expected a dictionary, but got str.',
            ]
        }
