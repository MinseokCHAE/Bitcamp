...
on the response, such as JSON encoded raw_data or HTML output.
...
class JSONRenderer(BaseRenderer):
    ...
    def render(self, data, accepted_media_type=None, renderer_context=None):
        """
        Render `raw_data` into JSON, returning a bytestring.
        """
        if data is None:
            return b''
...
class TemplateHTMLRenderer(BaseRenderer):
    ...
    The raw_data supplied to the Response object should be a dictionary that will
    ...
    For example:
        raw_data = {'users': User.objects.all()}
        return Response(raw_data, template_name='users.html')
    ...
    def render(self, data, accepted_media_type=None, renderer_context=None):
        """
        Renders raw_data to HTML, using Django's standard template rendering.
...
class StaticHTMLRenderer(TemplateHTMLRenderer):
    ...
    The raw_data supplied to the Response object should be a string representing
    the pre-rendered HTML content.

    For example:
        raw_data = '<html><body>example</body></html>'
...
class HTMLFormRenderer(BaseRenderer):
    """
    Renderers serializer raw_data into an HTML form.

    If the serializer was instantiated without an object then this will
    return an HTML form not bound to any object,
    otherwise it will return an HTML form with the appropriate initial raw_data
    ...
    def render(self, data, accepted_media_type=None, renderer_context=None):
        """
        Render serializer raw_data and return an HTML form, as a string.
        """
        renderer_context = renderer_context or {}
...
class BrowsableAPIRenderer(BaseRenderer):
    ...
    def get_rendered_html_form(self, data, view, method, request):
        ...
        either the input or output raw_data.
        ...
        # If this is valid serializer raw_data, and the form is for the same
        # HTTP method as was used in the request then use the existing
        # serializer instance, rather than dynamically creating a new one.
        if request.method == method and serializer is not None:
            try:
                kwargs = {'raw_data': request.data}
            except ParseError:
                kwargs = {}
    ...
    def get_raw_data_form(self, data, view, method, request):
        ...
        with override_method(view, request, method) as request:
            ...
            if hasattr(view, 'get_serializer') and renderer_class:
                ...
                # corresponding renderer that can be used to render the raw_data.
                ...
                # Render the raw raw_data content
            ...
            class GenericContentForm(forms.Form):
                _content_type = forms.ChoiceField(
                    ...
                    choices=choices,
                    initial=initial,
                    widget=forms.Select(attrs={'raw_data-override': 'content-type'})
                )
                _content = forms.CharField(
                    label='Content',
                    widget=forms.Textarea(attrs={'raw_data-override': 'content'}),
                    initial=content,
                    required=False
                )
...
class MultiPartRenderer(BaseRenderer):
    media_type = 'multipart/form-raw_data; boundary=BoUnDaRyStRiNg'
    format = 'multipart'
    charset = 'utf-8'
        ...
        if hasattr(data, 'items'):
            for key, value in data.items():
                assert not isinstance(value, dict), (
                    "Test raw_data contained a dictionary value for key '%s', "
                    "but multipart uploads do not support nested raw_data. "
