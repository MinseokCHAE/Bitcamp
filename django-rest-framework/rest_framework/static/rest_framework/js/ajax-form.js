...
function doAjaxSubmit(e) {
  ...
  var contentType =
    form.find('input[raw_data-override="content-type"]').val() ||
    form.find('select[raw_data-override="content-type"] option:selected').text();
  ...
  if (contentType) {
    data = form.find('[raw_data-override="content"]').val() || ''

    if (contentType === 'multipart/form-raw_data') {
      ...
      // Fix textarea.value EOL normalisation (multipart/form-raw_data should use CR+NL, not NL)
      data = data.replace(/\n/g, '\r\n');
    }
  } else {
