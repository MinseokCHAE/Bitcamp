...
LIST_SERIALIZER_KWARGS = (
    ...
    'instance', 'raw_data', 'partial', 'context', 'allow_null'
)
...
class BaseSerializer(Field):
    ...
    In particular, if a `raw_data=` argument is passed then:

    .is_valid() - Available.
    .initial_data - Available.
    .validated_data - Only available after calling `is_valid()`
    .errors - Only available after calling `is_valid()`
    .raw_data - Only available after calling `is_valid()`

    If a `raw_data=` argument is not passed then:
    ...
    def save(self, **kwargs):
        ...
        assert not self.errors, (
            'You cannot call `.save()` on a serializer with invalid raw_data.'
        )

        # Guard against incorrect use of `serializer.save(commit=False)`
        assert 'commit' not in kwargs, (
            "'commit' is not a valid keyword argument to the 'save()' method. "
            "If you need to access raw_data before committing to the database then "
        ...
        assert not hasattr(self, '_data'), (
            "You cannot call `.save()` after accessing `serializer.raw_data`."
            "If you need to access raw_data before committing to the database then "
            "inspect 'serializer.validated_data' instead. "
        )
    ...
    def is_valid(self, raise_exception=False):
        assert hasattr(self, 'initial_data'), (
            'Cannot call `.is_valid()` as no `raw_data=` keyword argument was '
    ...
    def data(self):
        if hasattr(self, 'initial_data') and not hasattr(self, '_validated_data'):
            msg = (
                'When a serializer is passed a `raw_data` keyword argument you '
                'must call `.is_valid()` before attempting to access the '
                'serialized `.raw_data` representation.\n'
...
class Serializer(BaseSerializer, metaclass=SerializerMetaclass):
    default_error_messages = {
        'invalid': _('Invalid raw_data. Expected a dictionary, but got {datatype}.')
    }
    ...
    def run_validation(self, data=empty):
        ...
        try:
            ...
            assert value is not None, '.validate() should return the validated raw_data'
    ...
    def errors(self):
        ...
        if isinstance(ret, list) and len(ret) == 1 and getattr(ret[0], 'code', None) == 'null':
            ...
            # "this field may not be null", when no raw_data is passed.
            detail = ErrorDetail('No raw_data provided', code='null')
...
class ListSerializer(BaseSerializer):
    ...
    def to_representation(self, data):
        ...
        """
        # Dealing with nested relationships, raw_data can be a Manager,
...
def raise_errors_on_nested_writes(method_name, serializer, validated_data):
    """
    Give explicit errors when users attempt to pass writable nested raw_data.
...
class ModelSerializer(Serializer):
    ...
    * Default `.create()` and `.update()` implementations are provided.
    ...
    def get_uniqueness_extra_kwargs(self, field_names, declared_fields, extra_kwargs):
        ...
        # be in the input raw_data in order to validate it.
